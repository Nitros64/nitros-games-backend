...
readonly secret_arn="arn:aws:secretsmanager:$region:$account_id:secret:nitros-games-backend/production/database/application-AbCd12"
...
if [[ "$1 $2" == "ecr get-login-password" ]]; then
  ...
  printf '{"username":"nitros_app","password":"test-$-password"}'
else
  exit 1
fi
EOF
...
if grep -q 'RDS_MASTER_SECRET_ARN\|secret:rds!' "$deploy_script" "$TEST_AWS_LOG"; then
  echo "Normal deployment still references the RDS master credential." >&2
  exit 1
fi
